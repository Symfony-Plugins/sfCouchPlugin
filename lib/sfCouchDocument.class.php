...
class sfCouchDocument
{
  ...
  public function __isset ($property)
  {
    ...
    if (array_key_exists($property, $this->properties ) || in_array($property, self::$specialProperties)) {
      return true;
    }
  ...
  public function checkRequirements ()
  {
    ...
    foreach ($this->requiredProperties as $property) {
      if (!isset($this->storage->$property ) || ($this->storage->$property === null)) {
        $errors[] = $property;
      }
    }
  ...
  public function save ()
  {
    ...
    if ($this->checkRequirements() !== true) {
      throw new sfException("Required properties for this document aren't set.");
    }

    // Check if we need to store the stuff at all
    if (($this->modified === false) && ($this->newDocument !== true)) {
      return false;
    }
    ...
    if (!isset($this->storage->_attachments ) || !count($this->storage->_attachments )) {
    ...
    if ($this->storage->_id === null) {
      ...
      $response = $db->post(json_encode($this->storage ));
    }
    else {
      // Store document in database
      $response = $db->put(urlencode($this->_id ), json_encode($this->storage ));
    }
  ...
  public function delete ()
  {
    if (!$this->isNew()) {
      $db = sfCouchConnection::getInstance();
      $response = $db->delete($this->_id . '?rev=' . urlencode($this->_rev));
      return $response;
    }
  }
}
